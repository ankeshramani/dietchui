...
class AddNewSupplierContact extends Component {
  ...
  addNewDepartmentContact = e => {
    ...
    if (selectedContactId && Number(selectedContactId) > -1) {
      ...
      const data = {
        ...contact,
        FKey_OP_Department: selectedDepartment.PKey_OP_Department ,
        ToBeEmailed
      }
  ...
  addNewContact = e => {
    ...
    this.props.form.validateFields(async (err, values) => {
      ...
      const {selectedDepartment, contactManager} = this.props;
      this.setState({
        isSaving: true,
      });
      const payload = {
  ...
  render() {
    ...
    const { ToBeEmailed, FirstName, isContactModal, LastName } = this.state;
      ...
      >
        <div className="pt-19">
          { !isContactModal && <DropDownBox
            ...
            displayExpr={(item) => item && `${item.FirstName }`}
            ...
            contentRender={()=>{
              return (
                <CustomGrid
                  ...
                  <Column alignment="left" width={"15%"} headerCellRender={() => <span className="mr-10 text-primary cursor-pointer" onClick={this.isModalOpen}>New</span>}/>
                </CustomGrid>
              )
            }}
          />}
