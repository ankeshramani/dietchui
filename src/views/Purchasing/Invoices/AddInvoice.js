...
class AddInvoiceItem extends Component {
  ...
  state = {
    ...
    invoiceId: this.props.match.params.invoiceId !== 'new' ? this.props.match.params.invoiceId : '',
    departments: [],
    isEditMode: this.props.match.params.invoiceId === 'new',
    suppliers: [],
    invoiceDetails: [],
    paymentTypes: [],
    supplierBids: [],
  ...
  getInvoiceData = async () => {
    ...
    const data = await this._apiService.getInvoiceData(invoiceId);
  ...
  getDeliveryHistory = async () => {
    ...
    const data = await this._apiService.getDeliveryHistory(invoiceId)
    if(!data || data.error){
      message.error('Something went wrong. Please try again later!')
      this.setState({
        loading: false,
  ...
  submitInvoice = async () => {
    ...
    const focusedRowKey = this.props.match.params.invoiceId;
    this.setState({
      isSaving: true,
    });
    const payload = {
    ...
    const data = await this._apiService.addUpdateInvoice(payload);
    ...
    if (!data || data.error) {
      ...
      this.props.history.push('/purchasing/invoices', {focusedRowKey})
    }
    this.setState({
      ...newState,
    });
  ...
  submitInvoiceDetails = async (FKey_OP_Invoice) => {
    ...
    const payload = invoiceDetails.map(x => {
      return {...x, FKey_OP_Supplier: invoiceData.FKey_OP_Supplier, FKey_OP_Invoice}
    });
    const data = await this._apiService.addUpdateMultipleInvoiceDetails(payload);
    const newState = {isSaving: false};
    if (!data || data.error) {
    ...
    if (this.state.invoiceDetails.length) {
      Modal.confirm({
        content: `There are bids in this invoice and you are trying to change the supplier.
              By changing the supplier you will be removing all bids from this invoice. Would you like to continue?`,
        okText: 'Yes',
        cancelText: 'Cancel',
        onOk: () => {
          this.setState({
  ...
  render() {
    const {isSaving, loading, invoiceId, invoiceData, isEditMode, departments = [], supplierBidLoading,
                      ...
                      <Row>
                        <Col md="12" sm="12" className="pr-5">
                          <Card className="mb-10">
                            <CardHeader>Invoice Information</CardHeader>
                            <CardBody>
                              <Row>
                                <Col md="6" sm="12">
                        ...
                        <Col md="12" className="px-10">
                          <TabsComp defaultActiveKey="1">
                            <TabPane tab="Details" key="1">
                              <Row>
                                <Col md="12" sm="12">
                                  <InvoiceDetails supplierBidLoading={supplierBidLoading} invoiceId={invoiceId} saveInvoiceDetails={this.saveInvoiceDetails} isEditable={isEditMode} FKey_OP_Supplier={invoiceData.FKey_OP_Supplier} supplierBids={supplierBids}/>
                                </Col>
                              </Row>
                            </TabPane>
...
const AddInvoice = Form.create()(AddInvoiceItem)
