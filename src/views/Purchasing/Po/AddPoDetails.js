...
class AddPoDetails extends Component {
  ...
  componentDidMount() {
    if (this.props.poId && this.props.poId !== 'new') {
      this.getGetByPo()
    }
  }
  ...
  getGetByPo = async () =>{
    ...
    const data = await this._apiService.getGetByPo(this.props.poId)
    if(!data || data.error){
  ...
  onDeleteRecord = async (selectedRecord) => {
    if (selectedRecord.PKey_OP_PO_Detail && String(selectedRecord.PKey_OP_PO_Detail) !== '-1') {
      const data = await this._apiService.deletePoDetail(selectedRecord.PKey_OP_PO_Detail)
      if (!data || data.error){
        ...
        this.setState({
          invoiceDetails: invoiceDetails.filter(x => x.PKey_OP_PO_Detail !== selectedRecord.PKey_OP_PO_Detail)
        })
      }
    }
    this.setState({
      invoiceDetails: this.state.invoiceDetails.filter(x => x.PKey_OP_PO_Detail !== selectedRecord.PKey_OP_PO_Detail)
    });
    this.refreshGrid()
  }
                      ...
                      loading ?  <Loader className="mt-50"/> :
                        ...
                        >
                              ...
                              return (
                                <div>
                                  <DropDownBox
                                    ...
                                    dataSource={this.props && this.props.supplierBids}
                                    contentRender={() => {
                                      return (
                                        <CustomGrid
                                          refCallback={(dg) => this.dg = dg}
                          ...
                          <Column width={100} headerCellRender={() => <span className="mr-10 text-primary cursor-pointer" onClick={this.onNewInvoiceDetail}>New</span>} visible={!!this.props.isEditable} dataField={''} cellRender={(record) => {
                            const menu = (
                              <Menu>
                                <Menu.Item onClick={() => this.onDeleteRecord(record.data)}>
