...
class Podetails extends React.Component {
  ...
  constructor(props) {
    ...
    this.getGetByPo(props.data.key);
  }

  getGetByPo = async (poId) =>{
    this.setState({
      loading: true,
    });
    const data = await this._apiService.getGetByPo(poId)
    if(!data || data.error){
  ...
  render() {
    const {poDetailsList, loading} = this.state;
    return (
      <React.Fragment>
        {
          loading ?  <Loader className="mt-50"/> :
            <DataGrid
              dataSource={poDetailsList}
              showBorders={false}
